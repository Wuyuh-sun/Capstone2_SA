<?
  ...
  $sql = "select * from faq_main where placename='$placename' order by idx desc";
  ...
  <li>
    <div>
      <a href='faq.php?placename={$placename}&title={$row['title']}&idx={$row['idx']}'><h3>{$row['title']}</h3></a>
  ...
  $bbs_form = "";
  if(isset($placename)){
    $noticeSql = "select * from faq_main where placename='$placename' and notice=1 order by idx desc";
    $noticeSql_result = mysqli_query($conn, $noticeSql);
    $notice_row = mysqli_fetch_array($noticeSql_result);

    $goodSql = "select * from faq_main where placename='$placename' and qna=1 order by idx desc";
    $goodSql_result = mysqli_query($conn, $goodSql);
    $hot_row = mysqli_fetch_array($goodSql_result);
    $bbs_form = "
    <div class='bbs_form'>
    <h1 class='bbs_title'>건의함 - {$placename}</h1>
    <a href='faq.php?placename={$placename}&title={$notice_row['title']}&idx={$notice_row['idx']}'>
      <div class='bbs_notice'>
        <img src='../../img/bell.png'> 공지 필독
      </div>
    </a>
    <a href='faq.php?placename={$placename}&title={$hot_row['title']}&idx={$hot_row['idx']}'>
      <div class='bbs_hot'>
  ...
  if(isset($_GET["write"])){
    if($_SESSION['user_grade'] == 'root'){
      $bbs_form = "";
      $write_form = "
      <form class='bbs_form' name='bbs_form' action='faq_write_insert.php' method='POST'>
        <a href='faq.php?placename={$placename}' class='closeBtn'></a>
        <div class='writetext'>글 쓰기</div>
        <label class='notice_chk'><input type='checkbox' name='notice' value=1>공지</label>
        <label class='qna_chk'><input type='checkbox' name='qna' value=1>자주 묻는 qna</label>
  ...
  if(isset($_GET['title'])){
    ...
    $sql = "select * from faq_main where placename='$placename' AND title='{$_GET['title']}' AND idx='{$_GET['idx']}'";
    $result = mysqli_query($conn, $sql);
    $row2 = mysqli_fetch_array($result);
    

    //댓글 리스트
    $comm_sql = "select * from faq_comm where bbs_idx='{$_GET['idx']}' AND placename='$placename' AND title='{$_GET['title']}'";
    $comm_result = mysqli_query($conn, $comm_sql);
    $comm_list = '';
    while($comm_row = mysqli_fetch_array($comm_result)){

      //대댓글 리스트
      $comm_comm_sql="select * from faq_comm_comm 
      where 
      ...
      <form action='faqComm_comm_insert.php' class='comm_comm_form comm_comm_form{$comm_row['idx']}' method='POST'>
    ...
    $good_sql = "
    select * from faq_good_data where click_user='{$_SESSION['usernickname']}' AND placename='$placename' AND title='{$_GET['title']}' AND content_idx='{$_GET['idx']}'
    ";
    ...
    <div class='contentBox1'>
      ...
      <form action='faq_read_delete.php' name='bbs_read_delete' method='POST'>
    ...
    <form action='faq_comm_insert.php' name='' class='comm_form' method='POST'>
  ...
  <script>
    ...
    if(isset($_GET['update'])){
      ...
      $bbsRead_form = "
    <div class='bbs_form'>
    <form action='faq_read_update.php' name='bbs_form' method='POST'>
      <h1 class='bbs_title'>건의함 - {$placename}</h1>
